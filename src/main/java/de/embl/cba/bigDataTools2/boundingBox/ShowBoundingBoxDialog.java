...
public class ShowBoundingBoxDialog {
    ...
    public void show(RandomAccessibleInterval rai, final String buttonName) {
        final int[] min, max;
        ...
        final Interval initialInterval = Intervals.createMinMax(minBB[BB_X_POS], minBB[BB_Y_POS], minBB[BB_Z_POS], maxBB[BB_X_POS], maxBB[BB_Y_POS], maxBB[BB_Z_POS]); // the initially selected bounding box
        final Interval rangeInterval = Intervals.createMinMax(min[BB_X_POS], min[BB_Y_POS], min[BB_Z_POS], max[BB_X_POS], max[BB_Y_POS], max[BB_Z_POS]);// the range (bounding box of possible bounding boxes)

        final AtomicBoolean lock = new AtomicBoolean(false);
        final int boxSetupId = 8888; // some non-existing setup id
        final BoundingBoxDialog boundingBoxDialog =
                new BoundingBoxDialog(((BdvHandleFrame) bdv).getBigDataViewer().getViewerFrame(), "Select 3D section to crop ",
                        ((BdvHandleFrame) bdv).getBigDataViewer().getViewer(),
                        ((BdvHandleFrame) bdv).getBigDataViewer().getSetupAssignments(), boxSetupId, initialInterval, rangeInterval) {
