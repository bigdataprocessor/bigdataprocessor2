...
public class PhaseCorrelation {
	...
	public static <T extends ComplexType<T>, S extends ComplexType<S>, R extends RealType<R>> RandomAccessibleInterval<R> calculatePCMInPlace(
		...
		//RandomAccessibleInterval<R> res = factory.create(realSize);
	...
	public static <T extends ComplexType<T> & NativeType<T>, S extends ComplexType<S> & NativeType <S>, R extends RealType<R>> RandomAccessibleInterval<R> calculatePCM(
		...
		try
		{
			...
			fft1Copy = factory.imgFactory( typeT ).create(fft1);
			//fft2Copy = factory.imgFactory( typeS ).create(fft2, typeS );
			fft2Copy = factory.imgFactory( typeS ).create(fft2);
		}
	 ...
	 */
	public static <T extends RealType<T>, S extends RealType<S>, R extends RealType<R>> PhaseCorrelationPeak getShift(
			...
			long minOverlap, boolean subpixelAccuracy, boolean interpolateSubpixel, ExecutorService service)
	{
		...
		List<PhaseCorrelationPeak> peaks = PhaseCorrelation2Util.getPCMMaxima(pcm, service, nHighestPeaks, subpixelAccuracy);
		...
		Collections.sort(peaks, Collections.reverseOrder(new PhaseCorrelationPeak.ComparatorByCrossCorrelation()));
			...
			RandomAccessibleInterval<R> pcm, RandomAccessibleInterval<T> img1, RandomAccessibleInterval<S> img2)
	{
		...
		PhaseCorrelationPeak res = getShift(pcm, img1, img2, 5, 0, true, false, service);
		service.shutdown();
		return res;
	}
/*
