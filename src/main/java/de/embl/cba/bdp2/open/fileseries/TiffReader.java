...
*/
            ...
            ExecutorService executorService )
    {
        ...
        if( info[ zs ].fileTypeString.equals( FileSeriesFileType.TIFF_STACKS.toString() ) )
        {
            imp = readDataCubeFromTiff(directory, info, executorService, zs, ze, nz, dz, xs, xe, ys, ye);
        }
        else if( info[ zs ].fileTypeString.equals( FileSeriesFileType.TIFF_PLANES.toString()  ) )
        {
            imp = readDataCubeFromTiff(directory, info, executorService, zs, ze, nz, dz, xs, xe, ys, ye);
        }
                                          ...
                                          int xs, int xe, int ys, int ye)
    {
        ...
        // TODO: it is not necessary to allocate new, but could be filled
        ImageStack stack = ImageStack.create(nx, ny, nz, fi.bytesPerPixel * 8);
    ...
    public class readCroppedPlaneFromTiffIntoImageStack implements Runnable
    {
        ...
        public void run()
        {
            ...
            if ( hasStrips )
            {
                ...
                if ( ( fi.compression == COMPRESSION_NONE ) || ( fi.compression == 0 ) )
                {
                    // do nothing
                }
